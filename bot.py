...
def init_db():
    ...
    try:
        ...
        # Extra review/audit fields for the upgraded deposit workflow.
        for column_sql in (
            "ALTER TABLE deposit_requests ADD COLUMN reviewed_by INTEGER",
            "ALTER TABLE deposit_requests ADD COLUMN review_note TEXT",
        ):
            try:
...
def review_deposit(request_id, status, reviewed_by=None, note=""):
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute(
            "UPDATE deposit_requests SET status=?, reviewed_at=?, reviewed_by=?, review_note=? WHERE request_id=? AND status='pending'",
            (status, datetime.now().isoformat(), reviewed_by, note.strip(), request_id),
        )
...
def get_deposit_history(limit=30):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute(
        "SELECT request_id, user_id, plan_id, amount, reference, status, created_at, reviewed_at, reviewed_by, review_note FROM deposit_requests ORDER BY request_id DESC LIMIT ?",
        (limit,),
    )
    rows = c.fetchall()
    conn.close()
...
def has_pending_deposit(user_id, plan_id):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute("SELECT request_id FROM deposit_requests WHERE user_id=? AND plan_id=? AND status='pending' LIMIT 1", (user_id, plan_id))
    row = c.fetchone()
    conn.close()
...
def create_admin_panel_inline():
    ...
    rows = [
        (("Add Plan", "add_plan_init"), ("Manage Plans", "manage_plans")),
        (("Add Subscription", "add_subscription"), ("Remove Subscription", "remove_subscription")),
        (("Add Admin", "add_admin"), ("Remove Admin", "remove_admin")),
        (("Pending Deposits", "pending_deposits"), ("Deposit History", "deposit_history")),
        (("Deposit Settings", "payment_settings"), ("Database", "database_menu")),
        (("Broadcast", "broadcast"), ("Lock / Unlock", "toggle_lock")),
        (("Run All Scripts", "run_all_scripts"), ("Bot Stats", "stats")),
    ]
    for row in rows:
        markup.add(*[types.InlineKeyboardButton(label, callback_data=cb) for label, cb in row])
    return markup
...
def _logic_view_plans(message_or_call):
    ...
    if not plans:
        ...
        return
    bot.send_message(chat_id, "HOSTING PLANS\n━━━━━━━━━━━━━━━━━━━━\nSelect a plan to continue.")
    for plan_id, name, limit, price, duration, buy_link in plans:
        card = (f"PLAN: {name}\n━━━━━━━━━━━━━━━━━━━━\n"
                f"File Limit: {limit}\nDuration: {duration} days\nPrice: {price}\n"
                f"Payment: Admin-reviewed deposit\n━━━━━━━━━━━━━━━━━━━━")
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(types.InlineKeyboardButton(f"Deposit for {name}", callback_data=f"buy_plan_{plan_id}"))
...
def _logic_deposit(message):
    ...
    plans = get_all_plans()
    if not plans:
        bot.reply_to(message, "No hosting plans are available right now.")
        return
    pending = []
    for rid, uid, pid, amount, reference, created_at in get_pending_deposits():
        if uid == user_id:
            pending.append((rid, pid, amount, reference))
    text = "DEPOSIT CENTER\n━━━━━━━━━━━━━━━━━━━━\nChoose a hosting plan and submit your payment reference.\n"
    if pending:
        text += "\nPending requests: " + ", ".join(f"#{x[0]}" for x in pending[:5]) + "\n"
    markup = types.InlineKeyboardMarkup(row_width=1)
    for pid, name, limit, price, duration, _ in plans:
        markup.add(types.InlineKeyboardButton(f"{name} — {price}", callback_data=f"buy_plan_{pid}"))
...
def handle_callbacks(call):
    ...
    # --- Manual Deposit Workflow ---
        ...
        pending_id = has_pending_deposit(user_id, plan_id)
        bot.answer_callback_query(call.id)
        msg = (f"DEPOSIT REQUEST\n━━━━━━━━━━━━━━━━━━━━\nPlan: {name}\nFile Limit: {limit}\nDuration: {duration} days\nPlan Price: {price}\n\n"
               f"PAYMENT INSTRUCTIONS\n━━━━━━━━━━━━━━━━━━━━\n{get_payment_info()}\n\n"
               "After payment, submit the exact paid amount and transaction/reference ID. "
               "An admin will verify it before activation.")
        markup = types.InlineKeyboardMarkup(row_width=1)
        if pending_id:
            markup.add(types.InlineKeyboardButton(f"Pending Request #{pending_id}", callback_data="deposit_noop"))
        else:
            markup.add(types.InlineKeyboardButton("Submit Deposit", callback_data=f"submit_deposit_{plan_id}"))
        markup.add(types.InlineKeyboardButton("Back to Deposit", callback_data="deposit_center_cb"))
        bot.send_message(call.message.chat.id, msg, reply_markup=markup)

    elif data == "deposit_center_cb":
        bot.answer_callback_query(call.id)
        _logic_deposit(call)

    elif data == "deposit_noop":
        bot.answer_callback_query(call.id, "This deposit is already pending.", show_alert=True)
        ...
        pending_id = has_pending_deposit(user_id, plan_id)
        if pending_id:
            bot.answer_callback_query(call.id, f"Request #{pending_id} is already pending.", show_alert=True)
            return
        bot.answer_callback_query(call.id)
        msg = bot.send_message(call.message.chat.id,
            "Send deposit details in this format:\n\nAMOUNT | TRANSACTION/REFERENCE ID\n\nExample: 500 BDT | TX123456789\n\nOnly submit a real payment reference. Your request will be reviewed by an admin.")
    ...
    # --- Admin Callbacks ---
        ...
        if not rows:
            ...
            return
        bot.send_message(call.message.chat.id, f"PENDING DEPOSITS: {len(rows)}\n━━━━━━━━━━━━━━━━━━━━")
        for rid, uid, pid, amount, reference, created_at in rows[:30]:
            ...
            markup = types.InlineKeyboardMarkup(row_width=2)
            markup.add(types.InlineKeyboardButton("Approve", callback_data=f"approve_deposit_{rid}"),
                       types.InlineKeyboardButton("Reject", callback_data=f"reject_deposit_{rid}"))
            markup.add(types.InlineKeyboardButton("User Profile", callback_data=f"deposit_user_{uid}"))
            bot.send_message(call.message.chat.id, text, reply_markup=markup)

    elif data.startswith("deposit_user_") and user_id in admin_ids:
        uid = int(data.split("_")[2])
        sub = user_subscriptions.get(uid)
        files_count = get_user_file_count(uid)
        status = "No active subscription"
        if sub and sub["expiry"] > datetime.now():
            status = f"{sub.get('plan_name','Premium')} until {sub['expiry'].strftime('%Y-%m-%d %H:%M')}"
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, f"USER PROFILE\n━━━━━━━━━━━━━━━━━━━━\nUser ID: {uid}\nSubscription: {status}\nFiles: {files_count}")
        ...
        if not row or row[5] != "pending":
            ...
            return
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(types.InlineKeyboardButton("Confirm Approval", callback_data=f"approve_confirm_{rid}"),
                   types.InlineKeyboardButton("Cancel", callback_data="deposit_cancel"))
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, f"Confirm approval for deposit #{rid}?\nUser: {row[1]}\nAmount: {row[3]}\nReference: {row[4]}", reply_markup=markup)

    elif data == "deposit_cancel" and user_id in admin_ids:
        bot.answer_callback_query(call.id, "Cancelled.")
        try: bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception: pass

    elif data.startswith("approve_confirm_") and user_id in admin_ids:
        rid = int(data.split("_")[2])
        row = get_deposit(rid)
        if not row or row[5] != "pending":
            bot.answer_callback_query(call.id, "Already reviewed or not found.", show_alert=True)
        ...
        if not plan:
            ...
            return
        _, pname, _, price, duration, _ = plan
        ...
        if review_deposit(rid, "approved", user_id, "Approved by admin"):
            save_subscription(uid, pname, expiry)
            bot.answer_callback_query(call.id, "Approved.")
            try: bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
            except Exception: pass
            bot.send_message(call.message.chat.id, f"Deposit #{rid} approved. User {uid} -> {pname} until {expiry.strftime('%Y-%m-%d %H:%M')}.")
            try:
                bot.send_message(uid, f"DEPOSIT APPROVED\n━━━━━━━━━━━━━━━━━━━━\nPlan: {pname}\nPaid: {amount}\nReference: {reference}\nYour plan is active until {expiry.strftime('%Y-%m-%d %H:%M')}.")
            except Exception: pass
        ...
        if not row or row[5] != "pending":
            ...
            return
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(types.InlineKeyboardButton("Confirm Rejection", callback_data=f"reject_confirm_{rid}"),
                   types.InlineKeyboardButton("Cancel", callback_data="deposit_cancel"))
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, f"Confirm rejection for deposit #{rid}?\nUser: {row[1]}\nReference: {row[4]}", reply_markup=markup)

    elif data.startswith("reject_confirm_") and user_id in admin_ids:
        rid = int(data.split("_")[2])
        row = get_deposit(rid)
        if not row or row[5] != "pending":
            bot.answer_callback_query(call.id, "Already reviewed or not found.", show_alert=True)
            return
        if review_deposit(rid, "rejected", user_id, "Rejected by admin"):
            bot.answer_callback_query(call.id, "Rejected.")
            try: bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
            except Exception: pass
            bot.send_message(call.message.chat.id, f"Deposit #{rid} rejected.")
            try: bot.send_message(row[1], f"DEPOSIT REJECTED\n━━━━━━━━━━━━━━━━━━━━\nRequest: #{rid}\nReference: {row[4]}\nPlease contact support if you believe this was a mistake.")
            except Exception: pass

    elif data == "deposit_history" and user_id in admin_ids:
        bot.answer_callback_query(call.id)
        rows = get_deposit_history(30)
        if not rows:
            bot.send_message(call.message.chat.id, "No deposit history yet.")
            return
        text = "DEPOSIT HISTORY\n━━━━━━━━━━━━━━━━━━━━\n"
        for rid, uid, pid, amount, reference, status, created_at, reviewed_at, reviewed_by, note in rows:
            plan = get_plan_by_id(pid)
            pname = plan[1] if plan else "Deleted plan"
            text += f"#{rid} | {pname} | {amount} | {status.upper()} | User {uid}\n"
        bot.send_message(call.message.chat.id, text[:3900])

    elif data == "run_all_scripts" and user_id in admin_ids:
        bot.answer_callback_query(call.id)
        started = 0
        for uid, files_list in list(user_files.items()):
            for fname, ftype in list(files_list):
                if is_bot_running(uid, fname):
                    continue
                fpath = os.path.join(get_user_folder(uid), fname)
                if os.path.exists(fpath):
                    try:
                        if ftype == "js": run_js_script(fpath, uid, get_user_folder(uid), fname, call.message)
                        elif ftype == "py": run_script(fpath, uid, get_user_folder(uid), fname, call.message)
                        started += 1
                    except Exception: pass
        bot.send_message(call.message.chat.id, f"Run All complete. Started: {started}")

    elif data == "stats" and user_id in admin_ids:
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, f"BOT STATS\n━━━━━━━━━━━━━━━━━━━━\nActive Users: {len(active_users)}\nHosted Files: {sum(len(v) for v in user_files.values())}\nRunning Bots: {len(bot_scripts)}\nAdmins: {len(admin_ids)}\nPending Deposits: {len(get_pending_deposits())}")

    elif data == "broadcast" and user_id in admin_ids:
        bot.answer_callback_query(call.id)
        msg = bot.send_message(call.message.chat.id, "Send the broadcast message now. It will be sent to active users.")
...
def process_deposit_submission(message, plan_id):
    try:
        ...
        numeric = re.search(r"\d+(?:\.\d+)?", amount.replace(",", ""))
        if not numeric or float(numeric.group()) <= 0:
            raise ValueError("Enter a valid positive amount")
        if len(reference) < 3:
            raise ValueError("Reference ID is too short")
        plan = get_plan_by_id(plan_id)
        if not plan:
            raise ValueError("Plan no longer exists")
        if has_pending_deposit(message.from_user.id, plan_id):
            raise ValueError("You already have a pending request for this plan")
        request_id = create_deposit_request(message.from_user.id, plan_id, amount, reference)
        pname, price = plan[1], plan[3]
        bot.reply_to(message, f"DEPOSIT SUBMITTED\n━━━━━━━━━━━━━━━━━━━━\nRequest: #{request_id}\nPlan: {pname}\nPlan Price: {price}\nPaid: {amount}\nReference: {reference}\nStatus: PENDING\n\nAn admin will review your payment and notify you.")
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(types.InlineKeyboardButton("Approve", callback_data=f"approve_deposit_{request_id}"),
                   types.InlineKeyboardButton("Reject", callback_data=f"reject_deposit_{request_id}"))
        for aid in admin_ids:
            try:
                bot.send_message(aid, f"NEW DEPOSIT #{request_id}\n━━━━━━━━━━━━━━━━━━━━\nUser: {message.from_user.id}\nPlan: {pname}\nPlan Price: {price}\nPaid: {amount}\nReference: {reference}\nSubmitted: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", reply_markup=markup)
            except Exception as e:
                logger.warning(f"Could not notify admin {aid}: {e}")
    except Exception as e:
        bot.reply_to(message, f"Deposit submission failed: {e}")
...
def process_broadcast(message):
    ...
    if not text:
        bot.reply_to(message, "Broadcast text cannot be empty.")
        return
    success = failed = 0
    for uid in list(active_users):
        try:
            ...
            failed += 1
    bot.reply_to(message, f"Broadcast finished. Sent: {success} | Failed: {failed}")
...
if __name__ == "__main__":
    logger.info(f"Starting {APP_NAME} with upgraded deposit workflow...")
    keep_alive()
