...
# ============================================================
# V15: HARD PRIORITY ROUTE FOR USER PANEL "Refer & Earn"
# This must be registered before every generic/custom text handler.
# ============================================================
async def _refer_earn_priority_router(m):
    if not m or not m.text:
        return
    label = clean_button_text(m.text)
    if label not in {"Refer", "Refer & Earn"}:
        return
    await referral_message(m)

dp.message.register(
    _refer_earn_priority_router,
    F.text,
    lambda m: bool(m and m.text and clean_button_text(m.text) in {"Refer", "Refer & Earn"})
)
