...
# ============================================================
# V16: VERIFIED FIRST-ROUTE FOR USER PANEL "Refer & Earn"
# Inspection showed the previous V15 fix added another referral route while
# referral_message was already registered elsewhere. This version removes the
# duplicate routes and installs ONE exact route first, before every other
# message handler. Exceptions are also caught so the button can never appear
# completely dead without a server-side log.
# ============================================================
async def refer_earn_first_route(m):
    if not m or not getattr(m, "text", None):
        return

    label = " ".join(str(m.text).replace("\u200b", "").split()).strip()
    if label not in {"Refer & Earn", "Refer"}:
        return

    try:
        return await referral_message(m)
    except Exception as e:
        print(f"[refer-earn] route error user={getattr(m.from_user, 'id', 'unknown')}: {e}")
        try:
            return await m.answer(
                "Refer & Earn খুলতে সমস্যা হয়েছে। আবার চেষ্টা করুন।",
                reply_markup=um(m.from_user.id)
            )
        except Exception as fallback_error:
            print(f"[refer-earn] fallback error: {fallback_error}")
            return None

dp.message.register(
    refer_earn_first_route,
    F.text,
    lambda m: bool(
        m and getattr(m, "text", None)
        and " ".join(str(m.text).replace("\u200b", "").split()).strip()
        in {"Refer & Earn", "Refer"}
    )
)
